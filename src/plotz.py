...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    sns.scatterplot(data=df, x="product_weight_g", y="freight_value").set(
        title="Freight Value and Weight Relationship"
    )


def plot_delivery_date_difference(df: DataFrame, save_to_file: str = None):
...
def plot_order_amount_per_day_with_holidays(df: DataFrame, save_to_file: str = None):
    ...
    ax1.plot(df.index, df['order_count'])
    holydays = df[df['holiday'] == 1].index.to_list()
    for day in holydays:
        ax1.axvline(x=day, color='r', linestyle=':')
