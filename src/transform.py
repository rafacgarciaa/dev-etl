...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    partial_data = orders.merge(items, left_on='order_id', right_on='order_id')
    data = partial_data.merge(products, left_on='product_id', right_on='product_id')
    ...
    aggregations['order_id'] = aggregations.index
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    holiday_dates = holidays['date'].map(lambda x: x.split(' ')[0]).tolist()

    result_df = pd.DataFrame({
        'order_count': order_purchase_ammount_per_date,
        'date': order_purchase_ammount_per_date.index.map(_to_timestamp),
        'holiday': order_purchase_ammount_per_date.index.map(lambda x: str(x)).isin(holiday_dates)
    })

    return QueryResult(query=query_name, result=result_df.sort_index(ascending=True))
...
def _to_timestamp(date):
    return pytz.utc.localize(datetime.strptime(str(date), "%Y-%m-%d"), is_dst=None).timestamp() * 1000